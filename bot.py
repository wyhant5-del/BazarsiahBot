...
API_TOKEN = '8967480237:AAG5vHg04VHi_ybk3ztSzZ5lvwTkCl9Eg-Q'
...
def get_player(user_id):
    ...
    if not player:
        ...
        conn.commit()
        return (user_id, 50000, 0, 0, 0)
    return player

def main_keyboard():
    kb = InlineKeyboardMarkup(row_width=2)
    kb.add(
        InlineKeyboardButton("💼 پروفایل و دارایی", callback_data="profile"),
        InlineKeyboardButton("📈 بازار سیاه", callback_data="market"),
        InlineKeyboardButton("🎰 ریسک و دزدی", callback_data="rob")
    )
    return kb
...
async def start_cmd(message: types.Message):
    ...
    await message.reply("💵 به بازی **سرمایه‌داری و بازار سیاه** خوش آمدی!\nهدف: تبدیل شدن به ثروتمندترین کاسب شهر.", reply_markup=main_keyboard(), parse_mode="Markdown")

@dp.callback_query_handler(lambda c: True)
async def process_callback(callback_query: types.CallbackQuery):
    user_id = callback_query.from_user.id
    data = callback_query.data
    _, cash, crypto, gold, b_level = get_player(user_id)

    if data == "profile":
        text = (
            f"👤 **شناسنامه سرمایه‌دار**\n\n"
            f"💰 پول نقد: {cash:,} تومان\n"
            f"🪙 ارز دیجیتال: {crypto} واحد\n"
            f"🥇 شمش طلا: {gold} عدد\n"
        )
        await bot.edit_message_text(text, user_id, callback_query.message.message_id, reply_markup=main_keyboard(), parse_mode="Markdown")
        ...
        await bot.edit_message_text(text, user_id, callback_query.message.message_id, reply_markup=kb, parse_mode="Markdown")
        ...
        if cash >= price:
            ...
            conn.commit()
            await callback_query.answer("✅ ۱ واحد ارز خریدی!")
        else:
        ...
        if crypto >= 1:
            cursor.execute('UPDATE players SET cash = cash + ?, crypto = crypto - 1 WHERE user_id = ?', (price, user_id))
            conn.commit()
            await callback_query.answer("✅ ۱ واحد ارز فروختی!")
        else:
